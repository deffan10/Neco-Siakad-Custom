...
@section('custom-css')
    ...
    <style>
        ...
        #barangTable_wrapper .row:first-child {
            margin-bottom: 1rem;
        }
    ...
    <div class="row">
        ...
        <div class="col-lg-8 col-12 mb-2">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">{{ $pages ?? 'Daftar Barang' }}</h5>
                    <div>
                        @if($is_trash)
                            <a href="{{ route($activeRole . '.inventaris.barang-index') }}" class="btn btn-sm btn-secondary me-2">
                                <i class="fas fa-arrow-left me-2"></i>Kembali ke Daftar Utama
                            </a>
                        @else
                            <a href="{{ route($activeRole . '.inventaris.barang-trash') }}" class="btn btn-sm btn-warning me-2">
                    ...
                    @if(!$is_trash)
                        <div class="collapse" id="collapseForm">
                            <div class="card card-body border">
                                <h5 class="card-title mb-3">Tambah Barang Baru</h5>
                                <form action="{{ route($activeRole . '.inventaris.barang-store') }}" method="post">
                                    @csrf
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-select" name="kategori_id" id="kategori_id" required>
                                                ...
                                                @foreach($kategori_barangs as $kategori)
                                                    <option value="{{ $kategori->id }}">{{ $kategori->name }} ({{ $kategori->code }})</option>
                                                @endforeach
                                            </select>
                                            @error('kategori_id')
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input type="text" class="form-control" name="code" id="code" placeholder="Masukkan kode barang" required>
                                            @error('code')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input type="text" class="form-control" name="merk" id="merk" placeholder="Masukkan merk barang" required>
                                            @error('merk')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                            ...
                            <tbody>
                                @foreach ($barangs as $key => $item)
                                    <tr>
                                        ...
                                        <td data-label="Kategori">{{ $item->kategori ? $item->kategori->name : '-' }}</td>
                                        ...
                                        <td data-label="Aksi">
                                            @if($is_trash)
                                                <form action="{{ route($activeRole . '.inventaris.barang-restore', $item->id) }}" method="POST" class="d-inline restore-form">
                                                    @csrf
                                            ...
                                            @else
                                                ...
                                                <form action="{{ route($activeRole . '.inventaris.barang-destroy', $item->id) }}" method="POST" class="d-inline delete-form">
                                                    @csrf
                                                    @method('DELETE')
                                        ...
                                        <div class="modal fade" id="editModal{{ $item->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $item->id }}" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{ route($activeRole . '.inventaris.barang-update', $item->id) }}" method="post">
                                                        @csrf
                                                        @method('PATCH')
                                                        <div class="modal-body">
                                                            <div class="mb-3">
                                                                <label for="kategori_id_edit{{ $item->id }}" class="form-label">Kategori Barang</label>
                                                                <select class="form-select" name="kategori_id" id="kategori_id_edit{{ $item->id }}" required>
                                                                    <option value="">Pilih Kategori</option>
                                                                    @foreach($kategori_barangs as $kategori)
                                                                        <option value="{{ $kategori->id }}" {{ $kategori->id == $item->kategori_id ? 'selected' : '' }}>{{ $kategori->name }} ({{ $kategori->code }})</option>
                                                                    @endforeach
                                                                </select>
                                                                @error('kategori_id')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                            ...
                                                            <div class="mb-3">
                                                                <label for="name_edit{{ $item->id }}" class="form-label">Nama Barang</label>
                                                                <input type="text" class="form-control" name="name" id="name_edit{{ $item->id }}" value="{{ $item->name }}" required>
                                                                @error('name')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                                @enderror
                                                            </div>
                                                            <div class="mb-3">
                                                                <label for="code_edit{{ $item->id }}" class="form-label">Kode Barang</label>
                                                                <input type="text" class="form-control" name="code" id="code_edit{{ $item->id }}" value="{{ $item->code }}" required>
                                                                @error('code')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                                @enderror
                                                            </div>
                                                            <div class="mb-3">
                                                                <label for="merk_edit{{ $item->id }}" class="form-label">Merk</label>
                                                                <input type="text" class="form-control" name="merk" id="merk_edit{{ $item->id }}" value="{{ $item->merk }}" required>
                                                                @error('merk')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                                @enderror
                                                            </div>
                                                            ...
                                                            <div class="mb-3">
                                                                <label class="form-label">Status</label>
                                                                <div class="form-check form-switch">
                                                                    <input class="form-check-input" type="checkbox" name="is_active" id="is_active_edit{{ $item->id }}" value="1" {{ $item->is_active ? 'checked' : '' }}>
                                                                    <label class="form-check-label" for="is_active_edit{{ $item->id }}">Aktif</label>
                                                                </div>
                                                                @error('is_active')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                                @enderror
        ...
        <div class="col-lg-4 col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <p>Halaman ini digunakan untuk mengelola data referensi barang.</p>
                    <ul>
                        <li>Klik tombol "Tambah Barang" untuk menambahkan data baru</li>
            ...
            var table = $('#barangTable').DataTable({
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        filename: function() {
                            return 'Data_Barang_' + new Date().toISOString().slice(0,10);
                        }
                    },
                    {
                        extend: 'excel',
                    ...
                    },
                    {
                        ...
                        customize: function(doc) {
                            ...
                            doc.content[0].text = 'Data Barang';
                    ...
                    },
                    {
                        ...
                        customize: function(win) {
                            $(win.document.body)
                                .css('font-size', '10pt')
                                .prepend('<div style="text-align:center; margin-bottom: 20px;"><h3>Data Barang</h3><p>Dicetak pada: ' + new Date().toLocaleDateString('id-ID') + '</p></div>');
