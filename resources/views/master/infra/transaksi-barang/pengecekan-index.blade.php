    ...
    <div class="row">
        ...
        <div class="col-lg-8 col-12 mb-2">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">{{ $pages ?? 'Daftar Pengecekan Barang' }}</h5>
                    <div>
                        @if($is_trash)
                            <a href="{{ route($activeRole . '.transaksi-barang.pengecekan-index') }}" class="btn btn-sm btn-secondary me-2">
                                <i class="fas fa-arrow-left me-2"></i>Kembali ke Daftar Utama
                            </a>
                        @else
                            <a href="{{ route($activeRole . '.transaksi-barang.pengecekan-trash') }}" class="btn btn-sm btn-warning me-2">
                            ...
                            <button class="btn btn-sm btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#collapseForm" aria-expanded="false" aria-controls="collapseForm">
                                <i class="fas fa-plus-circle me-2"></i>Tambah Pengecekan
                            </button>
                        @endif
                    </div>
                </div>
                <div class="card-body">
                    ...
                    @if(!$is_trash)
                        <div class="collapse" id="collapseForm">
                            <div class="card card-body border">
                                <h5 class="card-title mb-3">Tambah Pengecekan Barang Baru</h5>
                                <form action="{{ route($activeRole . '.transaksi-barang.pengecekan-store') }}" method="post">
                                    @csrf
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input type="date" class="form-control" name="tanggal_cek" id="tanggal_cek" required>
                                            @error('tanggal_cek')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-select" name="hasil_kondisi" id="hasil_kondisi" required>
                                                <option value="">Pilih Hasil Kondisi</option>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered" id="pengecekanTable">
                            <thead>
                                <tr>
                                    <th>No</th>
                            ...
                            <tbody>
                                @foreach ($pengecekan as $key => $item)
                                    <tr>
                                        ...
                                        <td data-label="Tanggal Cek">{{ $item->tanggal_cek ? \Carbon\Carbon::parse($item->tanggal_cek)->format('d/m/Y') : '-' }}</td>
                                        ...
                                        <td data-label="Aksi">
                                            @if($is_trash)
                                                <form action="{{ route($activeRole . '.transaksi-barang.pengecekan-restore', $item->id) }}" method="POST" class="d-inline restore-form">
                                                    @csrf
                                            ...
                                            @else
                                                ...
                                                <form action="{{ route($activeRole . '.transaksi-barang.pengecekan-destroy', $item->id) }}" method="POST" class="d-inline delete-form">
                                                    @csrf
                                                    @method('DELETE')
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="editModalLabel{{ $item->id }}">Edit Pengecekan Barang</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <form action="{{ route($activeRole . '.transaksi-barang.pengecekan-update', $item->id) }}" method="post">
                                                        @csrf
                                                        @method('PATCH')
                                                        <div class="modal-body">
                                                            ...
                                                            <div class="mb-3">
                                                                <label for="tanggal_cek_edit{{ $item->id }}" class="form-label">Tanggal Cek</label>
                                                                <input type="date" class="form-control" name="tanggal_cek" id="tanggal_cek_edit{{ $item->id }}" value="{{ $item->tanggal_cek ? \Carbon\Carbon::parse($item->tanggal_cek)->format('Y-m-d') : '' }}" required>
                                                                @error('tanggal_cek')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                                @enderror
                                                            </div>
        ...
        <div class="col-lg-4 col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <p>Halaman ini digunakan untuk mengelola data pengecekan barang inventaris.</p>
                    <ul>
                        <li>Klik tombol "Tambah Pengecekan" untuk menambahkan data baru</li>
                        <li>Gunakan tombol "Edit" untuk mengubah hasil kondisi dan catatan</li>
...
@section('custom-js')
    ...
    <script>
        ...
        $(document).ready(function() {
            var table = $('#pengecekanTable').DataTable({
                dom: 'Bfrtip',
                buttons: [
                    {
                        extend: 'copy',
                    ...
                    },
                    {
                        ...
                        customize: function(doc) {
                            ...
                            doc.content[0].text = 'Data Pengecekan Barang';
                    ...
                    },
                    {
                        ...
                        title: 'Data Pengecekan Barang',
                        customize: function(win) {
                            $(win.document.body)
                                .css('font-size', '10pt')
                                .prepend('<div style="text-align:center; margin-bottom: 20px;"><h3>Data Pengecekan Barang</h3><p>Dicetak pada: ' + new Date().toLocaleDateString('id-ID') + '</p></div>');
