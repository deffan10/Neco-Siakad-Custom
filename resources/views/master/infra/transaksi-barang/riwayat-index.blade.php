    ...
    <div class="row">
                    ...
                    <div>
                        ...
                        @else
                            ...
                            <button class="btn btn-sm btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#collapseForm" aria-expanded="false" aria-controls="collapseForm">
                                <i class="fas fa-plus-circle me-2"></i>Tambah Riwayat
                            </button>
                        @endif
                    </div>
                </div>
                                ...
                                <form action="{{ route($activeRole . '.transaksi-barang.riwayat-store') }}" method="post">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            <label for="pengajuan_id" class="form-label">Pengajuan (Opsional)</label>
                                            <select class="form-select" name="pengajuan_id" id="pengajuan_id">
                                                <option value="">Pilih Pengajuan (Opsional)</option>
                                                @foreach($pengajuan_list as $pengajuan)
                                                    <option value="{{ $pengajuan->id }}">Pengajuan #{{ $pengajuan->id }} - {{ $pengajuan->barangInventaris->barang->name }} ({{ $pengajuan->barangInventaris->nomor_inventaris }})</option>
                                                @endforeach
                                            </select>
                                            @error('pengajuan_id')
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input type="date" class="form-control" name="tanggal_service" id="tanggal_service" required>
                                        ...
                                        <div class="col-md-6 mb-3">
                                            <label for="tempat_service" class="form-label">Tempat Service</label>
                                            <input type="text" class="form-control" name="tempat_service" id="tempat_service" placeholder="Vendor / Bengkel / Internal">
                                            @error('tempat_service')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <label for="biaya" class="form-label">Biaya (Rp)</label>
                                            <input type="number" class="form-control" name="biaya" id="biaya" min="0" step="0.01">
                                            @error('biaya')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                            ...
                            <tbody>
                                @foreach ($riwayat as $key => $item)
                                    <tr>
                                        ...
                                        <td data-label="Pengajuan">{{ $item->pengajuan ? 'Pengajuan #' . $item->pengajuan->id : '-' }}</td>
                                        ...
                                        <td data-label="Tanggal Service">{{ $item->tanggal_service ? \Carbon\Carbon::parse($item->tanggal_service)->format('d/m/Y') : '-' }}</td>
                                        <td data-label="Tanggal Selesai">{{ $item->tanggal_selesai ? \Carbon\Carbon::parse($item->tanggal_selesai)->format('d/m/Y') : '-' }}</td>
                                        <td data-label="Tempat Service">{{ $item->tempat_service ?? '-' }}</td>
                                        <td data-label="Biaya (Rp)">{{ $item->biaya ? 'Rp ' . number_format($item->biaya, 2, ',', '.') : '-' }}</td>
                                        <td data-label="Hasil">
                                            @if($item->hasil == 'Baik')
                                                <span class="badge bg-success">{{ $item->hasil }}</span>
                                            @elseif($item->hasil == 'Rusak Ringan')
                                                <span class="badge bg-warning">{{ $item->hasil }}</span>
                                            @elseif($item->hasil == 'Rusak Berat')
                                                <span class="badge bg-danger">{{ $item->hasil }}</span>
                                            @elseif($item->hasil == 'Tidak Bisa Diperbaiki')
                                                <span class="badge bg-dark">{{ $item->hasil }}</span>
                                            @endif
                                        </td>
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{ route($activeRole . '.transaksi-barang.riwayat-update', $item->id) }}" method="post">
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="mb-3">
                                                                <label for="pengajuan_id_edit{{ $item->id }}" class="form-label">Pengajuan (Opsional)</label>
                                                                <select class="form-select" name="pengajuan_id" id="pengajuan_id_edit{{ $item->id }}">
                                                                    <option value="">Pilih Pengajuan (Opsional)</option>
                                                                    @foreach($pengajuan_list as $pengajuan)
                                                                        <option value="{{ $pengajuan->id }}" {{ $pengajuan->id == $item->pengajuan_id ? 'selected' : '' }}>Pengajuan #{{ $pengajuan->id }} - {{ $pengajuan->barangInventaris->barang->name }} ({{ $pengajuan->barangInventaris->nomor_inventaris }})</option>
                                                                    @endforeach
                                                                </select>
                                                                @error('pengajuan_id')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                            ...
                                                            <div class="mb-3">
                                                                <label for="tanggal_service_edit{{ $item->id }}" class="form-label">Tanggal Service</label>
                                                                <input type="date" class="form-control" name="tanggal_service" id="tanggal_service_edit{{ $item->id }}" value="{{ $item->tanggal_service ? \Carbon\Carbon::parse($item->tanggal_service)->format('Y-m-d') : '' }}" required>
                                                                @error('tanggal_service')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                                @enderror
                                                            </div>
                                                            <div class="mb-3">
                                                                <label for="tanggal_selesai_edit{{ $item->id }}" class="form-label">Tanggal Selesai</label>
                                                                <input type="date" class="form-control" name="tanggal_selesai" id="tanggal_selesai_edit{{ $item->id }}" value="{{ $item->tanggal_selesai ? \Carbon\Carbon::parse($item->tanggal_selesai)->format('Y-m-d') : '' }}">
                                                                @error('tanggal_selesai')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                                @enderror
                                                            </div>
                                                            <div class="mb-3">
                                                                <label for="tempat_service_edit{{ $item->id }}" class="form-label">Tempat Service</label>
                                                                <input type="text" class="form-control" name="tempat_service" id="tempat_service_edit{{ $item->id }}" value="{{ $item->tempat_service }}" placeholder="Vendor / Bengkel / Internal">
                                                                @error('tempat_service')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                                @enderror
                                                            </div>
                                                            <div class="mb-3">
                                                                <label for="biaya_edit{{ $item->id }}" class="form-label">Biaya (Rp)</label>
                                                                <input type="number" class="form-control" name="biaya" id="biaya_edit{{ $item->id }}" value="{{ $item->biaya }}" min="0" step="0.01">
                                                                @error('biaya')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                                @enderror
                                                            </div>
                                                            <div class="mb-3">
                                                                <label for="hasil_edit{{ $item->id }}" class="form-label">Hasil</label>
                                                                <select class="form-select" name="hasil" id="hasil_edit{{ $item->id }}" required>
                                                                    <option value="">Pilih Hasil</option>
                                                                    <option value="Baik" {{ $item->hasil == 'Baik' ? 'selected' : '' }}>Baik</option>
                                                                    <option value="Rusak Ringan" {{ $item->hasil == 'Rusak Ringan' ? 'selected' : '' }}>Rusak Ringan</option>
                                                                    <option value="Rusak Berat" {{ $item->hasil == 'Rusak Berat' ? 'selected' : '' }}>Rusak Berat</option>
                                                                    <option value="Tidak Bisa Diperbaiki" {{ $item->hasil == 'Tidak Bisa Diperbaiki' ? 'selected' : '' }}>Tidak Bisa Diperbaiki</option>
                                                                </select>
                                                                @error('hasil')
                                                                    <small class="text-danger">{{ $message }}</small>
                                                                @enderror
        ...
        <div class="col-lg-4 col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <p>Halaman ini digunakan untuk mengelola data riwayat perbaikan barang inventaris.</p>
                    <ul>
                        <li>Klik tombol "Tambah Riwayat" untuk menambahkan data baru</li>
                        <li>Gunakan tombol "Edit" untuk mengubah informasi riwayat perbaikan</li>
            ...
            var table = $('#riwayatTable').DataTable({
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        filename: function() {
                            return 'Data_Kategori_Barang_' + new Date().toISOString().slice(0,10);
                        }
                    },
                    {
                        extend: 'excel',
                    ...
                    },
                    {
                        ...
                        customize: function(doc) {
                            ...
                            doc.content[0].text = 'Data Riwayat Perbaikan';
                    ...
                    },
                    {
                        ...
                        title: 'Data Riwayat Perbaikan',
                        customize: function(win) {
                            $(win.document.body)
                                .css('font-size', '10pt')
                                .prepend('<div style="text-align:center; margin-bottom: 20px;"><h3>Data Riwayat Perbaikan</h3><p>Dicetak pada: ' + new Date().toLocaleDateString('id-ID') + '</p></div>');
                ...
                initComplete: function() {
                    ...
                    $('.dt-buttons').hide();
                    $('#kategoriBarangTable_length').hide();
                }
            });
