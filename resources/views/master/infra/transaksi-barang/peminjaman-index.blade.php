    ...
    <div class="row">
        ...
        <div class="col-lg-8 col-12 mb-2">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">{{ $pages ?? 'Daftar Peminjaman Barang' }}</h5>
                    <div>
                        @if($is_trash)
                            <a href="{{ route($activeRole . '.transaksi-barang.peminjaman-index') }}" class="btn btn-sm btn-secondary me-2">
                                <i class="fas fa-arrow-left me-2"></i>Kembali ke Daftar Utama
                            </a>
                        @else
                            <a href="{{ route($activeRole . '.transaksi-barang.peminjaman-trash') }}" class="btn btn-sm btn-warning me-2">
                    ...
                    @if(!$is_trash)
                        <div class="collapse" id="collapseForm">
                            <div class="card card-body border">
                                <h5 class="card-title mb-3">Tambah Peminjaman Barang Baru</h5>
                                <form action="{{ route($activeRole . '.transaksi-barang.peminjaman-store') }}" method="post">
                                    @csrf
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-select" name="peminjam_id" id="peminjam_id" required>
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input type="datetime-local" class="form-control" name="tanggal_pinjam" id="tanggal_pinjam" required>
                            ...
                            <tbody>
                                @foreach ($peminjaman as $key => $item)
                                    <tr>
                                        ...
                                        <td data-label="Peminjam">{{ $item->peminjam ? $item->peminjam->name : '-' }}</td>
                                        <td data-label="Petugas">{{ $item->petugas ? $item->petugas->name : '-' }}</td>
                                        <td data-label="Tanggal Pinjam">{{ $item->tanggal_pinjam ? \Carbon\Carbon::parse($item->tanggal_pinjam)->format('d/m/Y H:i') : '-' }}</td>
                                        <td data-label="Tanggal Kembali">{{ $item->tanggal_kembali ? \Carbon\Carbon::parse($item->tanggal_kembali)->format('d/m/Y H:i') : '-' }}</td>
                                        <td data-label="Tanggal Dikembalikan">{{ $item->tanggal_dikembalikan ? \Carbon\Carbon::parse($item->tanggal_dikembalikan)->format('d/m/Y H:i') : '-' }}</td>
                                        ...
                                        <td data-label="Aksi">
                                            @if($is_trash)
                                                <form action="{{ route($activeRole . '.transaksi-barang.peminjaman-restore', $item->id) }}" method="POST" class="d-inline restore-form">
                                                    @csrf
                                            ...
                                            @else
                                                ...
                                                <form action="{{ route($activeRole . '.transaksi-barang.peminjaman-destroy', $item->id) }}" method="POST" class="d-inline delete-form">
                                                    @csrf
                                                    @method('DELETE')
                                            ...
                                            <div class="modal-dialog modal-lg">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="editModalLabel{{ $item->id }}">Edit Peminjaman Barang</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <form action="{{ route($activeRole . '.transaksi-barang.peminjaman-update', $item->id) }}" method="post">
                                                        @csrf
                                                        @method('PATCH')
                                                        <div class="modal-body">
                                                            <div class="row">
                                                                ...
                                                                <div class="col-md-6 mb-3">
                                                                    <label for="peminjam_id_edit{{ $item->id }}" class="form-label">Peminjam</label>
                                                                    <select class="form-select" name="peminjam_id" id="peminjam_id_edit{{ $item->id }}" required>
                                                                        <option value="">Pilih Peminjam</option>
                                                                        @foreach($users as $user)
                                                                            <option value="{{ $user->id }}" {{ $user->id == $item->peminjam_id ? 'selected' : '' }}>{{ $user->name }}</option>
                                                                        @endforeach
                                                                    </select>
                                                                    @error('peminjam_id')
                                                                        <small class="text-danger">{{ $message }}</small>
                                                                ...
                                                                <div class="col-md-6 mb-3">
                                                                    <label for="tanggal_pinjam_edit{{ $item->id }}" class="form-label">Tanggal Pinjam</label>
                                                                    <input type="datetime-local" class="form-control" name="tanggal_pinjam" id="tanggal_pinjam_edit{{ $item->id }}" value="{{ $item->tanggal_pinjam ? \Carbon\Carbon::parse($item->tanggal_pinjam)->format('Y-m-d\TH:i') : '' }}" required>
                                                                    @error('tanggal_pinjam')
                                                                        <small class="text-danger">{{ $message }}</small>
                                                                    @enderror
                                                                </div>
                                                                <div class="col-md-6 mb-3">
                                                                    <label for="tanggal_kembali_edit{{ $item->id }}" class="form-label">Tanggal Kembali</label>
                                                                    <input type="datetime-local" class="form-control" name="tanggal_kembali" id="tanggal_kembali_edit{{ $item->id }}" value="{{ $item->tanggal_kembali ? \Carbon\Carbon::parse($item->tanggal_kembali)->format('Y-m-d\TH:i') : '' }}">
                                                                    @error('tanggal_kembali')
                                                                        <small class="text-danger">{{ $message }}</small>
                                                                    @enderror
                                                                </div>
                                                                <div class="col-md-6 mb-3">
                                                                    <label for="tanggal_dikembalikan_edit{{ $item->id }}" class="form-label">Tanggal Dikembalikan</label>
                                                                    <input type="datetime-local" class="form-control" name="tanggal_dikembalikan" id="tanggal_dikembalikan_edit{{ $item->id }}" value="{{ $item->tanggal_dikembalikan ? \Carbon\Carbon::parse($item->tanggal_dikembalikan)->format('Y-m-d\TH:i') : '' }}">
                                                                    @error('tanggal_dikembalikan')
                                                                        <small class="text-danger">{{ $message }}</small>
                                                                    @enderror
                                                                </div>
                                                                <div class="col-md-6 mb-3">
                                                                    <label for="status_edit{{ $item->id }}" class="form-label">Status</label>
                                                                    <select class="form-select" name="status" id="status_edit{{ $item->id }}" required>
                                                                        <option value="">Pilih Status</option>
                                                                ...
                                                                <div class="col-md-12 mb-3">
                                                                    ...
                                                                    <textarea class="form-control" name="keterangan" id="keterangan_edit{{ $item->id }}" rows="3">{{ $item->keterangan }}</textarea>
                                                                    @error('keterangan')
                                                                        <small class="text-danger">{{ $message }}</small>
                                                                    @enderror
                                                                </div>
        ...
        <div class="col-lg-4 col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <p>Halaman ini digunakan untuk mengelola data peminjaman barang inventaris.</p>
                    <ul>
                        <li>Klik tombol "Tambah Peminjaman" untuk menambahkan data baru</li>
                        <li>Gunakan tombol "Edit" untuk mengubah tanggal kembali, tanggal dikembalikan, status, dan keterangan</li>
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        customize: function(doc) {
                            ...
                            doc.content[0].text = 'Data Peminjaman Barang';
                    ...
                    },
                    {
                        ...
                        customize: function(win) {
                            $(win.document.body)
                                .css('font-size', '10pt')
                                .prepend('<div style="text-align:center; margin-bottom: 20px;"><h3>Data Peminjaman Barang</h3><p>Dicetak pada: ' + new Date().toLocaleDateString('id-ID') + '</p></div>');
