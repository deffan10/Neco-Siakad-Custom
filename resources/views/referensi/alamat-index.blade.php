...
@section('custom-css')
    ...
    <style>
        ...
        #alamatTable_wrapper .row:first-child {
            margin-bottom: 1rem;
        }
        ...
        <div class="col-lg-8 col-12 mb-2">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        @if($is_trash)
                            <a href="{{ route($activeRole . '.referensi.alamat-index') }}" class="btn btn-sm btn-secondary me-2">
                                <i class="fas fa-arrow-left me-2"></i>Kembali
                            </a>
                        @else
                            <a href="{{ route($activeRole . '.referensi.alamat-trash') }}" class="btn btn-sm btn-warning me-2">
                ...
                <div class="card-body">
                    @if(!$is_trash)
                        <div class="collapse" id="collapseForm">
                            <div class="card card-body border">
                                ...
                                <form action="{{ route($activeRole . '.referensi.alamat-store') }}" method="post">
                                    @csrf
                                    <div class="row">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered" id="alamatTable">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Tipe</th>
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td data-label="Aksi">
                                            @if($is_trash)
                                                <form action="{{ route($activeRole . '.referensi.alamat-restore', $item->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                            ...
                                            @else
                                                <a href="#" data-bs-toggle="modal" data-bs-target="#editData{{ $item->id }}" class="btn btn-sm btn-primary me-1" data-bs-toggle="tooltip" title="Edit Alamat">
                                                    <i class="fas fa-edit me-1"></i> Edit
                                                </a>
                                                <form action="{{ route($activeRole . '.referensi.alamat-destroy', $item->id) }}" method="POST" class="d-inline" id="delete-form-{{ $item->id }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-sm btn-danger" data-confirm-delete="true" data-bs-toggle="tooltip" title="Hapus Alamat" onclick="confirmDelete('{{ $item->id }}')">
                                ...
                                @foreach($alamats->sortByDesc('created_at')->take(3) as $item)
                                    <div class="list-group-item">
                                        <div class="d-flex w-100 justify-content-between">
                                            <h6 class="mb-1">{{ $item->owner_name }}</h6>
...
@section('custom-js')
    ...
    <script>
        ...
        $(document).ready(function() {
            var table = $('#alamatTable').DataTable({
                dom: 'Bfrtip',
                buttons: [
                    {
                        extend: 'copy',
        ...
        function confirmDelete(code) {
            Swal.fire({
                ...
                text: "Data alamat yang dihapus tidak dapat dikembalikan!",
                icon: 'warning',
