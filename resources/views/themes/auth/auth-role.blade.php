                    ...
                    <div class="container-tight">
                        ...
                        <div class="card card-md">
                            <div class="card-body">
                                ...
                                <style>
                                    .role-card { cursor: pointer; border: 1px solid var(--tblr-border, #e9ecef); transition: box-shadow .12s ease, border-color .12s ease, background .12s ease; }
                                    .role-card .check { display: none; }
                                    .role-card.selected { border-color: var(--tblr-primary, #0455a4); background: rgba(4,85,164,0.03); }
                                    .role-card.selected .check { display: inline-block; }
                                    .role-card:focus { outline: none; box-shadow: 0 0 0 .2rem rgba(4,85,164,0.08); }
                                    .role-icon { width: 40px; height: 40px; flex: 0 0 40px; }
                                </style>

                                <div class="d-grid gap-3 mb-3" id="role-cards">
                                    @foreach ($roles as $role)
                                        <div>
                                            <div class="card role-card" data-role="{{ strtolower($role) }}" role="button" tabindex="0" aria-pressed="false">
                                                <div class="card-body">
                                                    <div class="d-flex align-items-center">
                                                        <div class="me-3">
                                                            {!! $roleMeta[strtolower($role)]['icon'] ?? '<svg class="role-icon"><circle cx="12" cy="12" r="10"/></svg>' !!}
                                                        </div>
                                                        <div class="flex-grow-1 text-start">
                                                            <h3 class="h5 mb-0">{{ ucfirst($role) }}</h3>
                                                            <p class="text-muted small mb-0">
                                                                {{ $roleMeta[strtolower($role)]['desc'] ?? 'Role tambahan' }}
                                                            </p>
                                                        </div>
                                                        <div class="ms-2 check">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" ...><path d="M20 6L9 17l-5-5"/></svg>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <form id="role-form" action="{{ route('auth.set-role') }}" method="post">
                                    @csrf
                                    <input type="hidden" name="role" id="selected-role" value="" />
                                    <div class="form-footer">
                                        <button type="submit" id="role-submit" class="btn btn-primary w-100" disabled>Pilih Role & Lanjutkan</button>
                                    </div>
                                </form>
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function selectCard(card) {
            ...
            // move focus to submit for quicker flow
            submitBtn.focus();
        }
        ...
        roleCards.forEach(card => {
            ...
            card.addEventListener('keydown', function(e) {
                ...
                if (e.key === 'ArrowDown' || e.key === 'ArrowRight') {
                    e.preventDefault();
                    const idx = roleCards.indexOf(this);
                    const next = roleCards[(idx + 1) % roleCards.length];
                    next.focus();
                }
                if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') {
                    e.preventDefault();
                    const idx = roleCards.indexOf(this);
                    const prev = roleCards[(idx - 1 + roleCards.length) % roleCards.length];
                    prev.focus();
                }
            });
        });
